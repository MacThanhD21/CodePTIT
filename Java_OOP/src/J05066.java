...
class NhanViennn {
    ...
    public NhanViennn(String id, String name) {
        this.id = id;
        this.name = name;
    }
...
public class J05066 {
    public static void main(String[] args) {
        ...
        ArrayList<NhanViennn> arr = new ArrayList<>();

        int cnt_GD = 0, cnt_TP = 0, cnt_PP = 0;

        for(int i = 0; i < n; i++) {

            NhanViennn x = new NhanViennn(sc.next(), sc.nextLine());
        ...
        Collections.sort(arr, new Comparator<NhanViennn>() {

            @Override
            public int compare(NhanViennn o1, NhanViennn o2) {
        ...
        while (t-- > 0) {
            ...
            for (NhanViennn tmp : arr) {
                if (tmp.getName().toLowerCase().contains(s.toLowerCase())) {
