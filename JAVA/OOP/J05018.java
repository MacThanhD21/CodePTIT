...
class bangDiem {
    ...
    public bangDiem(String id, String tenHS, double toan, double tiengViet, double ngoaiNgu, double vatLy,
            double hoaHoc, double sinhHoc, double lichSu, double dia, double GDCD, double congNghe) {
        this.id = id;
        this.tenHS = tenHS;
        Toan = toan;
    ...
    public double diemTB() {
        return (this.Toan * 2 + this.tiengViet * 2 + this.ngoaiNgu + this.vatLy + this.hoaHoc + this.sinhHoc
                + this.lichSu + this.Dia + this.GDCD + this.congNghe) / 12.0;
    }
...
public class J05018 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < t; i++) {
            sc.nextLine();
            bd.add(new bangDiem("HS" + String.format("%02d", i + 1), sc.nextLine(), sc.nextDouble(), sc.nextDouble(),
